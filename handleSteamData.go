...
func handleSteamData(s *state, cmd command) error {
	steamClient := &http.Client{Timeout: 30 * time.Second}
	fmt.Println(s.steamAPIKey, s.steamID)
	url := fmt.Sprintf("http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=%v&steamid=%v", s.steamAPIKey, s.steamID)
